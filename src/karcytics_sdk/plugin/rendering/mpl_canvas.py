...
def _mpl_canvas_base() -> type:  # noqa: ANN201
    """Lazily import and return ``FigureCanvasQTAgg``.

    Deferred so that plugins that never use ``LayeredMatplotlibCanvas``
    are not forced to have ``matplotlib`` installed in their venv.
    """
...
class LayeredMatplotlibCanvas(_mpl_canvas_base()):
    ...
    def _apply_data_layer(self, render_data: Any) -> None:
        _base_draw = super().draw  # captured here where super() has __class__ context
    ...
    def draw(self) -> None:
        ...
        """
        _base_draw = super().draw  # captured in method scope
        self.raster_lock.try_run(
            lambda: _base_draw(),
            self.draw,
        )
